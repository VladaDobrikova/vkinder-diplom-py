...
while True:
    ...
    if message == "начать":
        ...
        write_msg(user_id=user_id, message=f"""Привет, {name}! если хотите начать поиск анкет, введите 'поиск', для того, чтобы посмотреть избранное, введите 'показать избранные анкеты', для просмотра черного списка введите 'показать черный список'""")
        ...
        name = dict_cur_user[user_id]['first_name']
        write_msg(user_id, f"{name}, введите 'смотреть' чтобы просмотреть анкеты")
        ...
        if dict_for_view == {}:
            write_msg(user_id, f"Для повторного поиска введите 'Поиск'")
        else:
        ...
        name = dict_cur_user[user_id]['first_name']
        write_msg(user_id, f"{name}, чтобы продолжить просмотр введите 'смотреть'")
        ...
        if dict_favorites == {}:
            write_msg(user_id, "У вас нет избранных анкет")
        else:
            if favorites not in dict_favorites.keys():
                write_msg(user_id, f"Для просмотра сначала введите 'с начала списка избранных'")
            else:
        ...
        favorites = 1
        write_msg(user_id, "чтобы посмотреть избранное введите 'показать избранные анкеты'")


    elif message == "показать черный список":
        if dict_blacklist == {}:
            write_msg(user_id, "У вас нет анкет в ЧС")
        else:
            if black_list not in dict_blacklist.keys():
                write_msg(user_id, f"Для просмотра сначала введите 'с начала черного списка'")
            else:
        ...
        black_list = 1
        write_msg(user_id, "Открыть черный список?")
        ...
        write_msg(user_id, "Попробуйте ответить по-другому...")
